...
const HotelDetail = () => {
  ...
  const handleRoomChange = event => {
    ...
    if (selectedRoomDetails?.availability_status.toUpperCase() === 'OFF') {
      toast.error('Phòng đang đóng. Vui lòng chọn phòng khác.')
    }
  ...
  const handleCheckInDateChange = e => {
    const selectedDate = new Date(e.target.value) // Chuyển đổi giá trị nhập vào thành Date object
    const todayDate = new Date() // Ngày hiện tại

    // Đặt thời gian của hôm nay về 00:00:00 để so sánh chính xác chỉ ngày
    todayDate.setHours(0, 0, 0, 0)

    if (selectedDate >= todayDate) {
      setCheckInDate(e.target.value) // Cập nhật ngày check-in
    } else {
      setCheckInDate(todayDate.toISOString().split('T')[0]) // Đặt ngày về hôm nay và định dạng lại
  ...
  const calculatePrice = () => {
    const roomPrice =
      rooms.find(room => room.id === selectedRoom)?.new_price > 0
        ? rooms.find(room => room.id === selectedRoom)?.new_price
        : 0
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      if (response?.status === 201) {
        ...
        toast.error(response.data.message || 'Booking already exists')
      } else {
                    ...
                    >
                      {rooms.map(room => (
                        <MenuItem
                          ...
                          value={room.id}
                          disabled={
                            room.availability_status.toUpperCase() === 'OFF'
                          } // Disable if availability_status is OFF
                        >
                          {room.availability_status.toUpperCase() === 'OFF'
                            ? 'Hết phòng '
                            : ''}
                  ...
                  <Button
                    ...
                    disabled={
                      rooms
                        ...
                        ?.availability_status.toUpperCase() === 'OFF' || loading
                    }
                    sx={{
